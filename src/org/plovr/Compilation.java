...
import org.plovr.fam.FileAlterationMonitor;
...
public final class Compilation {
	...
	/**
	 * This is only assigned if compile() is called with a {@link Config} that
	 * is in {@link CompilationMode#RAW}.
	 */
	private String inputJsConcatenatedInOrder;
	
	private FileAlterationMonitor fam= FileAlterationMonitor.newInstance();
	
	private String cachedCode;

	private Compilation(List<JSSourceFile> externs, List<JSSourceFile> inputs,
	...
	public void compile(Config config) {
		this.compileStartTimestamp = System.currentTimeMillis();
		this.config = config;
	...
	private void compileRaw(Config config) {
		...
		// Need to have a dummy Result that appears to be a success (i.e., has
		// no
		// errors or warnings).
		this.result = new Result(new JSError[0], // errors
	...
	private void compile(Config config, Compiler compiler,
		...
		for (JSSourceFile input:inputs){
			...
			try {
				File file = new File(config.getConfigFile().getParentFile(),
						path);
				if (file.exists()) {
				    fam.watch(file);
				}
		...
		if (config.getTreatWarningsAsErrors() && result.warnings.length > 0) {
			...
			System.arraycopy(originalResult.errors, 0, errors, 0,
					originalResult.errors.length);
			System.arraycopy(originalResult.warnings, 0, errors,
					originalResult.errors.length,
					originalResult.warnings.length);

			this.result = new Result(errors, new JSError[0], /* warnings */
			originalResult.debugLog, originalResult.variableMap,
					...
					originalResult.sourceMap, originalResult.externExport,
					originalResult.cssNames, originalResult.idGeneratorMap);
		}
	}
	...
	public String getCompiledCode() {
		...
		String compiledCode = inputJsConcatenatedInOrder != null ? inputJsConcatenatedInOrder
				: compiler.toSource();
		...
		if (outputWrapper != null) {
			...
			if (pos >= 0) {
				compiledCode = outputWrapper.substring(0, pos)
						+ compiledCode
						+ outputWrapper.substring(pos
								+ outputWrapperMarker.length());
			} else {
	...
	public String getCodeForModule(String moduleName, boolean isDebugMode,
		...
		return getCodeForModule(moduleName, isDebugMode, moduleNameToUri,
				resetSourceMap);
	}

	/**
	 * This method always calls compiler.toSource(module), which means that it
	 * can be followed by a call to compiler.getSourceMap().appendTo(writer,
	 * module), though compiler.getSourceMap().reset() should be called
	 * immediately after when that is the case.
	 * 
	 * This method is sychronized in order to fix
	 * http://code.google.com/p/plovr/issues/detail?id=31. The problem is that
	 * if two modules are requested at the same time, it is often the case that
	 * compiler.toSource(module) is executing for the second module while the
	 * source map is still being modified for the first module, causing an
	 * IllegalStateException. Empirically, synchronizing this method appears to
	 * fix things, though a more provably correct (and minimal) solution should
	 * be sought.
	 */
	private synchronized String getCodeForModule(String moduleName,
			boolean isDebugMode, Function<String, String> moduleNameToUri,
		...
		boolean hasGlobalScopeName = !Strings.isNullOrEmpty(config
				.getGlobalScopeName())
		...
		if (!isRootModule) {
			builder.append("\n//@ sourceURL="
					+ moduleNameToUri.apply(moduleName));
		}
	...
	public String getCodeForRootModule(boolean isDebugMode,
			Function<String, String> moduleNameToUri) {
		return getCodeForModule(getRootModuleName(), isDebugMode,
				moduleNameToUri);
	}

	/**
	 * Writes out all of the module files. This method is only applicable when
	 * modules are used. This is expected to be used only with the build
	 * command.
	 * 
	 * @throws IOException
	 */
	public void writeCompiledCodeToFiles(
		...
		ModuleConfig moduleConfig = config.getModuleConfig();
		Map<String, File> moduleToOutputPath = moduleConfig
		...
		for (JSModule module : modules) {
			...
			String moduleCode = getCodeForModule(moduleName, isDebugMode,
					moduleNameToUri, resetSourceMap);
			...
			if (sourceMapPath != null) {
				Writer writer = Streams.createFileWriter(sourceMapPath + "_"
						+ moduleName, config);
		...
		if (moduleConfig.excludeModuleInfoFromRootModule()) {
			...
			final Function<String, String> fingerprintedModuleNameToUri = new Function<String, String>() {
				@Override
				public String apply(String moduleName) {
					String uri = moduleNameToUri.apply(moduleName);
					String fingerprint = moduleNameToFingerprint
							.get(moduleName);
					if (fingerprint != null) {
			...
			appendRootModuleInfo(writer, isDebugMode,
					fingerprintedModuleNameToUri);
			Closeables.close(writer, false);
		}
	}

	/**
	 * This takes a file path and inserts a fingerprint into its name.
	 * Currently, there is no support for including a token to indicate where
	 * the fingerprint should be inserted, so the fingerprint is inserted
	...
	static JsonObject createModuleInfo(ModuleConfig moduleConfig) {
		...
		for (String moduleName : moduleConfig.getModuleNames()) {
			...
			for (String module : moduleConfig.getModuleInfo(moduleName)
					.getDeps()) {
	...
	private static List<CompilationError> normalizeErrors(JSError[] errors,
		...
		for (JSError error : errors) {
			compilationErrors.add(new CompilationError(error,
	...
	public boolean isOutOfDate(){
		return config.isOutOfDate()
				|| fam.getLastModificationTimestamp() > compileStartTimestamp;
	}
